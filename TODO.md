...
## Query Re-execution
- [ ] When opening SQL in nvim (`s`), allow editing and re-running the query via nvim command
- [ ] Return to qmb with the new result after re-execution
- [ ] Navigate between previous results with `[` and `]`
- [ ] Cache query results per session (by job ID) for back/forward navigation
